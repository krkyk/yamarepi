class Recipe < ApplicationRecord
  ...
  def self.recipe_week_favorites
    ...
    Recipe.includes(:favorited_customers).sort do |a, b|
      b.favorited_customers.includes(:favorites).where(favorites: { created_at: from...to }).size <=>
        a.favorited_customers.includes(:favorites).where(favorites: { created_at: from...to }).size
    end
  end

  def count_favorites_weekly
    favorites.where(favorites: { created_at: (Time.current - 6.day).at_beginning_of_day...Time.current.at_end_of_day }).count
  end
